...
export const signUp = async (req, res) => {
    try {
        ...
        const accessToken = createJwt({ id: userId }, { expiresIn: process.env.JWT_DURATION });
...
export const signIn = async (req, res) => {
    try {
        ...
        const accessToken = createJwt({ id: user.id }, { expiresIn: process.env.JWT_DURATION });
...
export const refreshToken = async (req, res) => {
    ...
    try {
        ...
        const accessToken = createJwt({ email: user?.email }, { expiresIn: process.env.JWT_DURATION });
        const refreshToken = createJwt({ email: user?.email }, { expiresIn: "30d" });
        res.cookie("accessToken", accessToken);
        res.cookie("refreshToken", refreshToken);
        return res.status(200).end();
    }
